...
> - 크기가 큰 partition 을 유지 할 수 있지만, 너무 작은 크기의 patition 이 많이 발생 할 수 있다.
>> 너무 작은 크기의 partition 은 활용도가 떨어짐

- Worst-fit(최악 적합)
프로세스가 들어갈 수 있는 partition 중 가장 큰 곳 선
> - best fit 과 마찬가지로 모든 메모리 공간을 탐색해야하므로 탐색시간이 길어 오버헤드가 크다.
> - 작은 크기의 partition 발생을 줄일 수 있지만, 큰 크기의 partition 확보가 어려울 수 있다.

- Nex-fit(순차 최초 적합)
최초 적합처럼 프로세스가 들어갈 수 있는 첫번째 partition 을 선택
> - state table 에서 마지막으로 탐색한 위치부터 탐색
> - 메모리 영역의 사용 빈도 균등화
> - 모든 메모리를 탐색하지 않기때문에 오버헤드 낮음

##### External Fragmentation Issue
external fragmentation issue 가 발생했을 때, 어떻게 해결할 것인가?
- Coalescing holes (공간 통합)
프로세스가 메모리를 릴리즈하고 나갔을때, 남은 partition 들의 공간을 하나로 통합한다.
> state table 에서 단순히 두개의 엔트리를 합해주면 되는 작업이기 때문에 오버헤드가 낮다

- Storage Compaction (메모리 압축)
모든 빈 공간을 하나로 통합한다
> - 모든 프로세스를 재배치 해야함, 즉 재배치가 필요한 프로세스들은 중지해야함
> 이는 매우 많은 시스템 자원을 소비하게 된다.

오버헤드가 크기 때문에, 일정 시간 또는 특정 요청이 있을 때만 수행하는 것이 좋다.
